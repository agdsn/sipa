...
def bustimes_(
    ...
    stops set up in the config.
    """
    data: dict[str, str] = {stopname: get_bustimes(stopname)} if stopname else {
        stop: get_bustimes(stop, 4) for stop in settings.busstops
    }
    return templates.TemplateResponse(
        request,
        "bustimes.html",
        dict(stops=data, stopname=stopname),
    )
